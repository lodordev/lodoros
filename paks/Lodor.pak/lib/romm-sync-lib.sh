...
# (_ensure_resolv) and probes RomM once FOR LOGGING, then returns non-fatally so the engine
...
#
# HONESTY CONTRACT (task #134): the probe must see the ENGINE'S transport or say it can't.
#   - tier-1 (socks5_proxy config): the engine socks5h-dials through tailscaled — a kernel-path
#     nc probe CANNOT see that (a MagicDNS .ts.net host doesn't even resolve outside the
#     tunnel), so the tunnel state (BackendState=Running) IS the probe.
#   - unparsed host / no nc: the probe says "skipped" — it NEVER claims "RomM reachable".
# _REACH_DETAIL always carries one true sentence for the caller's log. rc 0 = verified-or-
# can't-tell (proceed quietly), rc 1 = POSITIVELY unreachable (still non-fatal to callers).
_REACH_DETAIL=""
_romm_reachable() {
	if command -v tailscale_is_tier1 >/dev/null 2>&1 && tailscale_is_tier1; then
		if [ "$(tailscale_status 2>/dev/null)" = "connected" ]; then
			_REACH_DETAIL="tier-1 tunnel Running (socks5 ${TS_SOCKS5_ADDR:-?}) — engine transport verified"
			return 0
		fi
		_REACH_DETAIL="tier-1 tunnel NOT Running — engine socks5 dial will fail (see tailscaled.log)"
		return 1
	fi
	if [ -z "${ROMM_HOST:-}" ]; then
		_REACH_DETAIL="probe skipped: host unparsed from config.json — engine connect is authoritative"
		return 0   # can't tell — never gate, never claim
	fi
	if ! command -v nc >/dev/null 2>&1; then
		_REACH_DETAIL="probe skipped: no nc on this firmware — engine connect is authoritative"
		return 0   # can't tell — never fabricate a problem (or an OK)
	fi
	if nc -z -w "${_REACH_TIMEOUT:-4}" "$ROMM_HOST" "${ROMM_PORT:-443}" >/dev/null 2>&1; then
...
# RETURN: 0 = transport verified (or probe honestly can't tell), 1 = positively unreachable.
# NON-FATAL either way — every caller proceeds to the engine for its own honest rc; the code
# only feeds truthful logging (_REACH_DETAIL). Control flow is unchanged from the always-0 era.
wifi_ensure_reachable() {
	_ensure_resolv
	# TIER-1 (task #134): a down TUNNEL is not a Wi-Fi wedge — the radio/gateway are typically
	# fine, so never gateway-probe or re-enum over it. _romm_reachable already probed the
	# engine's real transport (BackendState); report it truthfully and return.
	if command -v tailscale_is_tier1 >/dev/null 2>&1 && tailscale_is_tier1; then
		if _romm_reachable; then
			_wlog "reach: ok ($_REACH_DETAIL)"
			return 0
		fi
		_wlog "reach: $_REACH_DETAIL — radio NOT cycled (tunnel state is not a Wi-Fi wedge)"
		return 1
	fi
	if _romm_reachable; then
		_wlog "reach: ok ($_REACH_DETAIL)"
	...
	if ! _have_ip; then
		_REACH_DETAIL="no IP lease (bring-up case, not the have-IP wedge) — not re-enuming"
		_wlog "reach: $_REACH_DETAIL"
		_net_diag
		return 1
	fi
	if _reenum_recent; then
		_REACH_DETAIL="dead link, but re-enumed within ${_REENUM_COOLDOWN}s — skipping (manual reset if it persists)"
		_wlog "reach: $_REACH_DETAIL"
		_net_diag
		return 1
	fi
	...
	_wifi_recover_wedge
	if _romm_reachable; then
		_REACH_DETAIL="recovered after one-shot re-enum ($_REACH_DETAIL)"
		_wlog "reach: $_REACH_DETAIL"
		return 0
	fi
	_REACH_DETAIL="still unreachable after one re-enum — giving up (manual Wi-Fi reset needed)"
	_wlog "reach: $_REACH_DETAIL"
	_net_diag
	return 1
}
...
wifi_acquire() {
	...
	if [ "$PLAT" = miyoomini ] && grep -qs 0bda /sys/bus/usb/devices/*/idVendor 2>/dev/null; then
		true > "$ROMM_PAK_DIR/.wifi-8188fu" 2>/dev/null
	fi
...
_persist_clock() {
	[ -n "$DATETIME_PATH" ] && date +'%F %T' > "$DATETIME_PATH" 2>/dev/null
	# #147: best-effort hwclock write-back where an RTC node exists, so boards with
	# a battery-less-but-present RTC keep the time across a soft reboot.
...
# persist_clock_if_sane - the #147 persist CADENCE hook (syncd loop): refresh
# datetime.txt only when the clock reads a sane year, so a garbage epoch boot can
# never overwrite a good persisted time.
persist_clock_if_sane() {
	_yr=$(date +%Y 2>/dev/null)
	[ -n "$_yr" ] && [ "$_yr" -ge 2024 ] 2>/dev/null && _persist_clock
	return 0
}

# lodor_clock_restore - forward-only restore from datetime.txt (task #147). The
# miyoomini/my282/my355 boot entries carry this inline (earliest possible); on
# families whose boot entry is a prebuilt binary (rg35xxplus dmenu.bin) this runs
# at romm-syncd startup - the earliest tracked per-boot hook. Idempotent and
# FORWARD-ONLY: a stored time in the past never moves the clock backward (NTP via
# set_clock stays the only backward authority).
lodor_clock_restore() {
	[ -f "$DATETIME_PATH" ] || return 0
	_lcr_stored="$(head -n1 "$DATETIME_PATH" 2>/dev/null)"
	_lcr_now="$(date +'%F %T' 2>/dev/null)"
	_lcr_s="$(printf '%s' "$_lcr_stored" | tr -cd '0-9')"
	_lcr_n="$(printf '%s' "$_lcr_now" | tr -cd '0-9')"
	[ "${#_lcr_s}" -eq 14 ] && [ "${#_lcr_n}" -eq 14 ] && [ "$_lcr_s" != "$_lcr_n" ] || return 0
	# equal-length digit strings: lexicographic order == chronological order
	[ "$(printf '%s\n%s\n' "$_lcr_s" "$_lcr_n" | sort | head -n1)" = "$_lcr_n" ] || return 0
	if date -s "$_lcr_stored" >/dev/null 2>&1; then
		_wlog "clock restore: forward to $_lcr_stored (datetime.txt)"
		if command -v hwclock >/dev/null 2>&1 && { [ -e /dev/rtc0 ] || [ -e /dev/rtc ]; }; then
			hwclock -w >/dev/null 2>&1
		fi
	fi
...
# --- tier-1 (Tailscale) overlay (task #134 — mirrors the sibling NextUI lib wiring) ----------
# Load the Tailscale helpers (tailscale_up / _is_tier1 / _status / _reconnect ...) so EVERY
# entry point that sources this lib (romm-run, romm-syncd, the minarch shim via
# romm-session-sync, launch.sh) can bring the tunnel up — before #134 only launch.sh ("Sync
# Now") sourced tailscale-lib, so a tier-1 device booted with the engine's SOCKS5 proxy dead
# and every download failed (the 2026-07-02 RG34XX field logs). Guarded: a pak without the
# lib (miyoomini build) skips this and behaves exactly as before. Sourced AFTER
# ROMM_PAK_DIR / SDCARD / PLAT are set (it keys off them).
if [ -f "$ROMM_PAK_DIR/lib/tailscale-lib.sh" ]; then
	. "$ROMM_PAK_DIR/lib/tailscale-lib.sh" 2>/dev/null || true
fi

# lodor_tier1_up — bring the userspace tailnet up (reusing the QR-onboarded persisted login)
# ONLY when config.json is a tier-1 host (carries socks5_proxy). Idempotent (a running daemon
# is reused, never duplicated) + non-fatal (a LAN / public-URL config is a silent no-op; a
# failed bring-up just lets the engine surface its own unreachable rc 3). The daemon is left
# resident for the session; only the user (Tailscale.pak Turn off / Reset & forget) stops it.
lodor_tier1_up() {
...
run_sync() {
	...
	_ensure_resolv
	# tier-1 FIRST (task #134): the engine's socks5h dial needs a live tailscaled, and the
	# reach probe below reads the tunnel state on tier-1 configs — bring it up (or reuse it)
	# before either. No-op for LAN / public-URL configs; never blocks them.
	lodor_tier1_up
